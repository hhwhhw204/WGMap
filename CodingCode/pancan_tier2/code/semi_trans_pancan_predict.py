...
class Transformer(nn.Module):
    def __init__(self, n_input, ):
        ...
        self.attn1 = MultiHeadSelfAttention(dim_in=self.n, dim_k=4, dim_v=18)  # 18
...
def load_data(path, multiple):
    ...
    joblib.dump(ss, '../model/trans_predruggable.scaler')
    hg = df['geneid']
    targets = df['label']
...
def fit_cv(X, Y, lr, e1, e2, multiple, ratio):
    ...
    m = optimal_threshold
    joblib.dump(model, '../model/XGB_NoKFold.pkl')
    del model
    ...
    # plt.plot(mean_fpr, mean_tpr, color='#9467BD')
    # plt.show()
    # np.savetxt('../result/1_' +str(multiple) + '/pancan_tier2_auc.csv', np.column_stack((mean_fpr, mean_tpr)), delimiter=',')

    return roc_auc_1, pr1, pvalue, m


def predict(X_test, m, multiple):
    Hugo_list = list(X_test['Hugo_Symbol'])
    X_test.drop(['Hugo_Symbol'], axis=1, inplace=True)

    ss = joblib.load('../model/trans_predruggable.scaler')
    X_test = ss.transform(X_test)
    X_test = torch.tensor(X_test).to(torch.float32)

    model = joblib.load('../model/trans_pre.pkl')
...
def main():
    ...
    mean_auroc, mean_auprc, pvalue, m = fit_cv(x, y, args.lr, args.epoch1, args.epoch2, args.multiple,
                                          args.ratio)
    drug_num,drug_gene,drug_prob = predict(X_test, m, args.multiple)
