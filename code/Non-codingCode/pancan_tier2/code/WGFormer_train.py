...
def build_set(all_list):
  ...
  df_fu = df_neg_ids_noncds.sample(n=len(pos_ids)*10,random_state = 0,replace = False)#10
...
def load_data(path):
    ...
    joblib.dump(ss, f'{root_path}/code/Non-codingCode/pancan_tier2/model/trans_predruggable1.scaler')
    hg = df['geneid']
    targets = df['label']
...
def fit_cv(Xs, y, k, b_plot=False):
    z = 0
    n = Xs.shape[0]
    tprs = []
    p = []
    ...
    for j in range(1, k + 1):
        ...
        model = joblib.load(f'{root_path}/code/Non-codingCode/pancan_tier2/model/train/WGFormer_{j}.pkl')
        ...
        # 找到精确度和召回率差异最小的点
        # 这可以通过找到精确度和召回率之间差的最小值的索引来实现
        difference = np.abs(precision - 0.7)
        ...
        if roc_auc_1 > z:  # 准确率最大时，对应的门限值
            ...
            m1 = optimal_th
            m = optimal_threshold
            #joblib.dump(model,  f'{root_path}/code/Non-codingCode/pancan_tier2/model/WGFormer_train1.pkl')
        del model
...
def main(argv=sys.argv):
    ...
    feature_train.to_csv( f'{root_path}/code/Non-codingCode/pancan_tier2/data/trans_pre_CV.csv', index=False)

    #交叉验证
    x,y=cv_data(feature_train.shape[0])
    #print(x.shape,y.shape)
    m=fit_cv(x, y, 5, b_plot=False)
