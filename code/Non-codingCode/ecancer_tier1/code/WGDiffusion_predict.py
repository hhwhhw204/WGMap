...
def train(cancer_type, method='0'):
  df = pd.read_csv(f'{root_path}/code/Non-codingCode/ecancer_tier1/result/{cancer_type}_CV_tra_1.csv',header = 0,sep=',')
  y_train = df['label']
  y_train = np.array(y_train)
  features = df.drop(['geneid', 'label'], axis=1)
  X_train = np.array(features)
  df1 = pd.read_csv(f'{root_path}/code/Non-codingCode/ecancer_tier1/result/{cancer_type}_CV_val_1.csv' ,header = 0,sep=',')
  y_test = df1['label']
  ...
  joblib.dump(scaler, f'{root_path}/code/Non-codingCode/ecancer_tier1/model/{cancer_type}.xgbscaler_v')
  if method == 'XGB':
    model=xgb.XGBClassifier(random_state=0)
    scaler=joblib.load(f'{root_path}/code/Non-codingCode/ecancer_tier1/model/{cancer_type}.xgbscaler_v')
    ...
    m = optimal_threshold
    joblib.dump(model, f'{root_path}/code/Non-codingCode/ecancer_tier1/model/{cancer_type}_WGDiffusion_predict.pkl')
    del model
...
def predict(cancer_type,X_test,m,test_ids,X_train,Y_train,train_pos):
    scaler = joblib.load(f'{root_path}/code/Non-codingCode/ecancer_tier1/model/{cancer_type}/predict/{cancer_type}.xgbscaler_v')
    X_test = scaler.fit_transform(X_test)
    model = joblib.load(f'{root_path}/code/Non-codingCode/ecancer_tier1/model/{cancer_type}/predict/{cancer_type}_WGDiffusion_predict.pkl')
    probas_ = model.predict_proba(X_test)[:, 1]
    ...
    #drugable_id_score.to_csv(f'{root_path}/results/Non-codingCode/ecancer_tier1/{cancer_type}/targetGenes_Hcredible.csv', index=False)
...
def main(argv=sys.argv):
    ...
    feature_train.to_csv(f'{root_path}/code/Non-codingCode/ecancer_tier1/result/{args.type}_CV_1.csv', index=False)
    ...
    feature_train_tra.to_csv(f'{root_path}/code/Non-codingCode/ecancer_tier1/result/{args.type}_CV_tra_1.csv', index=False)
    feature_train_va=feature_train.iloc[length:]
    #print(feature_train_va.shape)
    feature_train_va.to_csv(f'{root_path}/code/Non-codingCode/ecancer_tier1/result/{args.type}_CV_val_1.csv', index=False)

    m = joblib.load(f'{root_path}/code/Non-codingCode/ecancer_tier1/model/{cancer_type}/predict/{cancer_type}_optimal_threshold.pkl')
    if args.mode == 'pred':
